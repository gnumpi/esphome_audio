...
void WishboneBus::dump_config(){
  esph_log_config(TAG, "Matrixio:");
    ...
    esph_log_config(TAG, "  Found Matrixio Voice with firmware: %u", (uint) this->matrixio_version );
  }
  else if (this->matrixio_id == MATRIX_CREATOR_ID){
    esph_log_config(TAG, "  Found Matrixio Creator with firmware: %u", this->matrixio_version );
  }
  else {
    esph_log_config(TAG, "  Matrixio not found! (id: %u, version: %u)", (uint) this->matrixio_id, this->matrixio_version);
  }
}
